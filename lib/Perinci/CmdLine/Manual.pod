...
Logging is done with L<Log::Any> (for producing). For displaying logs, P::C uses
L<Log::Any::App> currently which supports multiple outputs (screen, file, etc)
while P::C::Lite currently uses L<Log::Any::Adapter::ScreenColoredLevel> which
only supports displaying to screen.
...
=item * [P::C] If action metadata's default_log setting is defined, use that
...
P::C (as well as a few other frameworks) and generate shell completion scripts
