...
Logging is done with L<Log::Any> (for producing). For displaying logs,
PC::Classic uses L<Log::Any::App> currently which supports multiple outputs
(screen, file, etc) while PC::Lite currently uses
L<Log::Any::Adapter::ScreenColoredLevel> which only supports displaying to
screen.
