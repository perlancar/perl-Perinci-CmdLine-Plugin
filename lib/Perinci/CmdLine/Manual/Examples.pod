...
can specify binary data including NUL bytes.

In the C<text> output formats, P::C will also print the binary result from
server as-is without any newline added.
