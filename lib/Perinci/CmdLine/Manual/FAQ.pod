...
Currently only bash completion is supported. Basically, you activate completion
by invoking C<complete -C ProgramName ProgramName> in bash. This means, to get
completion bash will execute the program with COMP_LINE and COMP_POINT
environment set to the command-line and cursor position, respectively. P::C will
detect these environment variables and do the completion for you (either
...
Another alternative used by many is to generate C<complete> bash commands
statically.
